...
void traverseTheTree(SearchTree root, int nodeCount)
{
    ...
    //先将根节点入队
    Enqueue((int)root, myQueue);
    while (!IsEmpty(myQueue))
    {
        SearchTree front;
        //下面的实现在指针型与int之间发生了相互转换，其实是有风险的，但在本实验中并没有体现出影响
        //出队一个节点，查看其子节点
        front = (SearchTree)FrontAndDequeue(myQueue);
        printf("%d ", front->Element);
        //将出队结点的左子节点入队
        if (front->Left)
            Enqueue((int)(front->Left), myQueue);
        //将出队结点的右子节点入队
        if (front->Right)
            Enqueue((int)(front->Right), myQueue);
    }
}
